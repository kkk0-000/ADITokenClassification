"""
Multi-LABEL token prediction with LoRA + multi-GPU via Accelerate.

Launch examples:
  accelerate launch --num_processes 4 pred_Token.LoRA_multilabel_ddp.py -i input.csv --num_labels 3 ...
  python pred_Token.LoRA_multilabel_ddp.py -i input.csv --num_labels 3 ...
"""
...
def get_parameters():
    parser = argparse.ArgumentParser(description='Multi-label token prediction LoRA (DDP)')
    ...
    with torch.no_grad():
        for batch in dataloader:
            ...
            for j, idx in enumerate(idxs):
                ...
                seq = sequences[idx]
                seqlen = len(seq)
                token_logits = logits[j][1:seqlen + 1]
