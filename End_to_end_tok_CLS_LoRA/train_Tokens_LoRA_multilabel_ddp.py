"""
Multi-LABEL token classification with LoRA + DDP + Focal Loss.

Launch examples:
  accelerate launch --num_processes 4 train_Tokens_LoRA_multilabel_ddp.py \
      --num_labels 3 --loss_type focal --focal_gamma 2.0 --auto_pos_weight --ft_mode lora ...
  python train_Tokens_LoRA_multilabel_ddp.py --num_labels 3 ...
"""
...
# ---------------------------------------------------------------------------
def get_parameters():
    parser = argparse.ArgumentParser(description='Multi-LABEL token classifier LoRA (DDP)')
...
def model_load(model_name, num_labels, ft_mode, lora_rank):
    ...
    if ft_mode == 'lora':
        peft_config = LoraConfig(
            ...
            bias="none", use_rslora=True, r=lora_rank,
            lora_alpha=16 * (lora_rank ** .5), lora_dropout=0.05,
...
def train_args_prepare(args):
    ...
    folder_name = (f"finetune/{mol_name}-{args.ft_mode}-TokenCLS-multilabel-{args.num_labels}"
                   f"-{args.loss_type}-gamma{args.focal_gamma}-ddp")
    if args.ft_mode == 'lora':
        folder_name = (f"finetune/{mol_name}-lora-r{args.lora_rank}-TokenCLS-multilabel-{args.num_labels}"
                       f"-{args.loss_type}-gamma{args.focal_gamma}-ddp")
    return TrainingArguments(
        output_dir=folder_name, overwrite_output_dir=True,
        ...
        ddp_find_unused_parameters=False, dataloader_pin_memory=True,
...
def main():
    ...
    print(f'Loss: {args.loss_type}, gamma: {args.focal_gamma}, labels: {args.num_labels} (multi-label)')
    ...
    print('Loading model...')
    model, tokenizer = model_load(args.model_name, args.num_labels, args.ft_mode, args.lora_rank)
    ...
    trainer = MultilabelTrainer(
        ...
        loss_type=args.loss_type, pos_weight=pos_weight,
        focal_gamma=args.focal_gamma, num_labels=args.num_labels,
    )
