...
def get_parameters():
    parser = argparse.ArgumentParser(description='Multi-class token prediction')
...
def eval_data(dataloader, model, num_classes):
    ...
    for _, batch in enumerate(dataloader):
        ...
        for i in range(len(names)):
            ...
            probs = F.softmax(logits[i][1:seqlen+1], dim=1)
...
def get_blocks(pred):
    """Extract contiguous blocks of same label."""
    tags, starts, ends = [], [], []
...
def merge_predictions(all_results, class_names):
    """Extract predicted fragments for each non-background class."""
    predictions = []
...
def main():
    ...
    all_results = eval_data(dataloader, model, args.num_classes)
