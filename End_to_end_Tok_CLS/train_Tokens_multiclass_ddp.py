"""
Multi-class token classification with multi-GPU DDP + Focal Loss / class-weighted loss.

Launch examples:
  # 4 GPUs + Focal Loss:
  accelerate launch --num_processes 4 train_Tokens_multiclass_ddp.py \
      --num_classes 3 --loss_type focal --focal_gamma 2.0 --auto_class_weight ...

  # 4 GPUs + weighted CrossEntropy:
  accelerate launch --num_processes 4 train_Tokens_multiclass_ddp.py \
      --num_classes 3 --loss_type ce_weighted --auto_class_weight ...

  # torchrun:
  torchrun --nproc_per_node=4 train_Tokens_multiclass_ddp.py --num_classes 3 --loss_type focal ...

  # Single GPU fallback:
  python train_Tokens_multiclass_ddp.py --num_classes 3 ...
"""
...
# ---------------------------------------------------------------------------
# Custom Trainer: only override compute_loss, model stays as-is
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
def get_parameters():
    parser = argparse.ArgumentParser(description='Multi-class token classifier (DDP + Focal)')
...
# ---------------------------------------------------------------------------
# Model: uses original EsmForTokenClassification, no subclass needed
# ---------------------------------------------------------------------------
...
def train_args_prepare(args):
    ...
    folder_name = (f"finetune/{mod_name}-{args.ft_mode}-Token-multiclass-{args.num_classes}"
                   f"-{args.loss_type}-gamma{args.focal_gamma}-ddp")
    if args.ft_mode == 'lora':
        folder_name = (f"finetune/{mod_name}-lora-r{args.lora_rank}-Token-multiclass-{args.num_classes}"
                       f"-{args.loss_type}-gamma{args.focal_gamma}-ddp")
...
def main():
    ...
    # Load data first (needed for auto class weight)
    print('Loading data...')
    ...
    if args.class_weights:
        ...
        class_weights = compute_class_weights_from_labels(train_set, args.num_classes)

    # Load model (original EsmForTokenClassification, untouched)
    print('Loading model...')
    model, tokenizer, data_collator = model_load(
        args.model_name, args.num_classes, args.dropout,
    ...
    # Use FocalTrainer instead of Trainer — loss logic lives here
